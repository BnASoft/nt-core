...
$listHref = NT_URL.'/board/'.$id.'/p/'.$p;
if (!empty($qstr))
    $listHref .= '?'.http_build_query($qstr, '', '&amp;');

$editHref   = NT_URL.'/board/'.$id.'/edit/'.$no.'?'.$qry;
$replyHref  = NT_URL.'/board/'.$id.'/reply/'.$no.'?'.$qry;
$deleteHref = NT_URL.'/board/'.$id.'/delete/'.$no.'?'.$qry;
$writeHref  = NT_URL.'/board/'.$id.'/write';
