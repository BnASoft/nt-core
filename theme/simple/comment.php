...
<div class="mt-5 mb-4 border bg-light pl-3 pb-3">
    ...
    <ul class="list-unstyled col mt-3">
        ...
        for($i = 0; $row = array_shift($result); $i++) {
            $editButton   = '<a href="'.NT_URL.'/board/'.$id.'/'.$no.'/comment/'.$row['cm_no'].'/edit" class="comment-button text-muted" target="_blank"><i class="icon" data-feather="edit"></i></a>';
            $replyButton = '<a href="'.NT_URL.'/board/'.$id.'/'.$no.'/comment/'.$row['cm_no'].'/reply" class="ml-2 comment-button text-muted" target="_blank"><i class="icon" data-feather="plus-square"></i></a>';
            $deleteButton = '<a href="'.NT_URL.'/board/'.$id.'/'.$no.'/comment/'.$row['cm_no'].'/delete" class="ml-2 comment-button text-muted" target="_blank"><i class="icon" data-feather="x-square"></i></a>';
