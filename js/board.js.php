...
jQuery(function() {
    jQuery(document).on("click", "form.form-write button:submit, form.form-write input:submit, form.form-write input:image", function(e) {
        ...
        if(typeof(f.bo_name) != "undefined") {
            ...
            if(name.length < 1) {
                jQuery("#bo_name").data("content", "<?php echo _d('Please enter a Name.', THEME_LOCALE_DOMAIN); ?>").popover("show");
                return false;
            }
        }
        ...
        if(typeof(f.bo_password) != "undefined") {
            ...
            if(pass.length < 1) {
                jQuery("#cm_password").data("content", "<?php echo _d('Please enter a Password.', THEME_LOCALE_DOMAIN); ?>").popover("show");
                return false;
            }
        }

        if(subject.length < 1) {
            jQuery("#bo_subject").data("content", "<?php echo _d('Please enter a Subject.', THEME_LOCALE_DOMAIN); ?>").popover("show");
                return false;
        }

        if(content.length < 1) {
            jQuery("#bo-content-editor").data("content", "<?php echo _d('Please enter a Contents.', THEME_LOCALE_DOMAIN); ?>").popover("show");
                return false;
        }

        if (typeof(grecaptcha) != "undefined") {
            if (grecaptcha.getResponse() == "") {
                $f.find(".recaptcha_area").data("content", "<?php echo _d('Please check the anti-spam code.', THEME_LOCALE_DOMAIN); ?>").popover("show");
                return false;
            }
        }
