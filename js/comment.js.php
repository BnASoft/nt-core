...
{
    jQuery("#comment-area").load(nt_url+"/board/"+viewId+"/"+viewNo+"/comment");
}
...
jQuery(function() {
    ...
    jQuery(document).on("click", "form.form-comment button:submit, form.form-comment input:submit, form.form-comment input:image", function(e) {
        ...
        if(typeof(f.cm_name) != "undefined") {
            ...
            if(name.length < 1) {
                $f.find(".cm_name").data("content", "<?php echo _d('Please enter a Name.', THEME_LOCALE_DOMAIN); ?>").popover("show");
                return false;
            }
        }
        ...
        if(typeof(f.cm_password) != "undefined") {
            ...
            if(pass.length < 1) {
                $f.find(".cm_password").data("content", "<?php echo _d('Please enter a Password.', THEME_LOCALE_DOMAIN); ?>").popover("show");
                return false;
            }
        }

        if(w != "d" && content.length < 1) {
            $f.find(".cm_content").data("content", "<?php echo _d('Please enter a Contents.', THEME_LOCALE_DOMAIN); ?>").popover("show");
                return false;
        }
