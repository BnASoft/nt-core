...
if ($w == '') {
    if ((int)__c('cf_password_length') && strlen($mb_password) < (int)__c('cf_password_length'))
        die(json_encode(array('error' => sprintf(_n('Please enter your password at least %d character.', 'Please enter your password at least %d characters.'), (int)__c('cf_password_length')), 'element' => 'mb_password')));
...
if($w == '') {
    ...
    if (!$result)
        die(json_encode(array('error' => _('You can not sign up for an email is duplicated or other errors.'), 'element' => 'mb_email')));
    ...
    $mb = getMember($mb_uid, 'mb_uid, mb_password, mb_email');
    if(!$mb['mb_uid'])
    ...
    $sql = " update `{$nt['member_table']}` set mb_name = :mb_name, mb_email = :mb_email, mb_password = :mb_password where mb_uid = :mb_uid ";
